...
import { Camera, User as UserIcon, Loader2, ShieldCheck } from 'lucide-react'; // Changed icon for fallback
...
const adminProfileFormSchema = z.object({
  ...
  admin_level: z.coerce.number().int().min(1, {message: "Admin level must be at least 1"}).max(99, {message: "Admin level seems too high"}).optional().nullable(),
});
...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    if (user) {
      form.reset({
        ...
        gender: user.gender as AdminProfileFormValues['gender'] || 'Prefer not to say',
        address: user.address || '',
        contact: user.contact || '',
        admin_level: user.admin_level !== undefined ? Number(user.admin_level) : null,
      });
  ...
  const onSubmit = async (data: AdminProfileFormValues) => {
    ...
    if (data.admin_level !== null && data.admin_level !== undefined) {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/api/v1/users/me/admin`, {
        method: 'PUT',
        headers: {
      ...
      if (setUser) {
        setUser(prevUser => ({ ...prevUser, ...responseData, profile_image: responseData.profile_image || prevUser?.profile_image, admin_level: responseData.admin_level }));
      }
    ...
    <DashboardLayout>
      <div className="p-4 md:p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="text-2xl sm:text-3xl font-bold">Admin Account Settings</CardTitle>
            <CardDescription>
            ...
            <CardContent className="space-y-8">
              <div className="flex flex-col items-center space-y-4">
                <Avatar className="w-32 h-32 border-4 border-muted shadow-md">
                  <AvatarImage src={profileImagePreview || undefined} alt={user.full_name || 'Admin avatar'} />
                  <AvatarFallback className="text-4xl bg-gradient-to-br from-primary to-secondary text-primary-foreground">
                    {user.full_name ? user.full_name.split(' ').map(n => n[0]).join('').toUpperCase() : <ShieldCheck size={48} />}
                  </AvatarFallback>
                </Avatar>
                <Button 
                  type="button" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => form.setValue('gender', value as AdminProfileFormValues['gender'], { shouldValidate: true })}
                  >
                ...
                <div className="space-y-2">
                 ...
                 <div className="space-y-2 md:col-span-2">
                  <Label htmlFor="admin_level">Admin Level (Optional)</Label>
                  <Input 
                    id="admin_level" 
                    type="number"
                    {...form.register('admin_level')} 
                    placeholder="e.g., 1, 2, 3"
                  />
                  {form.formState.errors.admin_level && (
                    <p className="text-sm text-red-500">{form.formState.errors.admin_level.message}</p>
                  )}
                </div>
              </div>
            </CardContent>
