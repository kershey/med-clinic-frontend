...
import { Camera, User as UserIcon, Loader2, Users } from 'lucide-react'; // Changed icon for fallback
...
const staffProfileFormSchema = z.object({
  ...
  // Add other staff-specific editable fields here if any in the future
});
...
export default function StaffSettingsPage() {
  ...
  useEffect(() => {
    if (user) {
      form.reset({
        ...
        gender: user.gender as StaffProfileFormValues['gender'] || 'Prefer not to say',
  ...
  const onSubmit = async (data: StaffProfileFormValues) => {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/api/v1/users/me/staff`, {
        method: 'PUT',
        headers: {
      ...
      if (setUser) {
        setUser(prevUser => ({ ...prevUser, ...responseData, profile_image: responseData.profile_image || prevUser?.profile_image }));
      }
    ...
    <DashboardLayout>
      <div className="p-4 md:p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="text-2xl sm:text-3xl font-bold">Staff Account Settings</CardTitle>
            <CardDescription>
            ...
            <CardContent className="space-y-8">
              <div className="flex flex-col items-center space-y-4">
                <Avatar className="w-32 h-32 border-4 border-muted shadow-md">
                  <AvatarImage src={profileImagePreview || undefined} alt={user.full_name || 'Staff avatar'} />
                  <AvatarFallback className="text-4xl bg-gradient-to-br from-primary to-secondary text-primary-foreground">
                    {user.full_name ? user.full_name.split(' ').map(n => n[0]).join('').toUpperCase() : <Users size={48} />}
                  </AvatarFallback>
                </Avatar>
                <Button 
                  type="button" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => form.setValue('gender', value as StaffProfileFormValues['gender'], { shouldValidate: true })}
                  >
